...
public class RemotePeers {
	...
	public static void main(String[] args) {
		...
		for (PeerInfo remotePeer : peerList) {
			try {
				...
				((com.jcraft.jsch.Channel) channel).setInputStream(null);
				((ChannelExec) channel).setErrStream(System.err);

				InputStream input = ((com.jcraft.jsch.Channel) channel).getInputStream();
	...
	public void getConfiguration()
	{
		String st;
		int i1;
		Vector<RemPeerInfo> peerInfoVector;
		peerInfoVector = new Vector<RemPeerInfo>();
		try {
			...
			while((st = in.readLine()) != null) {
			     ...
			     peerInfoVector.addElement(new RemPeerInfo(tokens[0], tokens[1], tokens[2]));
